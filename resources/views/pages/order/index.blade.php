                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if($order->status === 'menunggu')
                                                <form action="{{ route('order.destroy', $order->id) }}" method="POST"
                                                    ...
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        onclick="return confirm('Yakin ingin menghapus order #{{ $order->id }}?')"
                                                        class="p-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 rounded-lg transition-colors"
                                                        title="Hapus Order">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
